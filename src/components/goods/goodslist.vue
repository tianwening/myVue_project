<template>
    <!-- 实现上拉刷新需要给mt-loadmore组件的父元素添加overflow:scroll的css属性 -->
    <div class="tmp scro" style="height:577px;">
        <nav-bar title="商品列表"></nav-bar>
        <mt-loadmore :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" :auto-fill="isAutoFill" ref="loadmore">
            <!-- onBottomLoaded函数是一个通知上拉完毕的通知 -->
...
export default {
    ...
    methods: {
        ...
        loadMore() {
            this.$ajax.get("getgoodslist/" + (this.pageIndex) + ".json").then(resp => {
                this.prods = resp.data;
            }).catch(err => {
                console.log(err);
            })
        },
        loadMoreConcat(){
            this.$ajax.get("getgoodslist/" + (++this.pageIndex) + ".json").then(resp => {
                this.prods = this.prods.concat(resp.data);
